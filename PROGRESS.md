...
- `ff792f4`: separated APK-dist outputs from AGP inputs after connected-test task validation failure; all variants verified locally. Current remote branch at this revision. Both subsequent hosted builds passed.
...
Stage6f: add PNG+UI hierarchy captures, phase-labelled lifecycle failure evidence, actual SystemUI notification tap and destination-synchronized Back assertions; wake the device before independent Shizuku checks. No assertions removed. Local `compileDebugAndroidTestKotlin assembleDebugAndroidTest` PASSED (log `/tmp/batstats-stage6f-compile.log`); diff checks pass. No active compiler. This stage is ready for commit and hosted execution.
Then reread this record; push held220b4d3/a01e24c plus stage6f under existing approval, and update PR1 body from docs/PR_DESCRIPTION.md through REST (`gh pr edit` uses a deprecated GraphQL field). Inspect both device phases, download per-phase reports/screenshots and visually review them. Fix actual failures, preserving assertions. Keep PR draft. Finish clean APK recollection/provenance and delivery records only after available device validation.
