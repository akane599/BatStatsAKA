...
- Stage6g widget regression committed d9f353d, compilation/Android test APK assembly passed1m13s; published with0e9b301. It uses actual system widget binding/delivery and scripted readings; not executed yet.
- Stage6h committed0e9b301 and published: private-cache PNG/XML capture followed by test-shell copy to /sdcard/Download/batstats-validation-screenshots; AGP/ADB collect that directory. Shizuku selector uses package plus verified English Allow all the time text from the pinned release. Eleven host tests, bash/actionlint/diff checks pass.
...
- PR1 body updated. Hosted push35331600751/job105556961354 and PR35331604578/job105556978396 are building0e9b301. Inspect ordinary and Shizuku reports/screenshots, fix actual failures without weakening assertions. Keep PR draft; merging/releases/settings/cancellation excluded.
- Current artifacts refreshed from clean0e9b301 after38s incremental assembly; signatures and16KiB ZIP alignment pass. Hashes/source/signer in artifacts/build-info.json. No physical measurement claim. After compaction read records/status/recent commits/all diffs before implementation. PR body uses REST; gh pr edit's deprecated GraphQL query fails.

Stage6i while CI builds: extend the existing dark/font200 navigation test through dashboard and observed-drain screens in landscape; restore natural orientation and unfreeze in cleanup. Compile/test APK build PASSED1m (/tmp/batstats-stage6i-landscape.log);28 methods still. No active compiler. Next: commit this test-only stage, hold its push for current hosted results, then review both phases and captures.
