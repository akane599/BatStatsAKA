...
- Stage6f committed0e7455a and pushed with220b4d3/a01e24c; PR1 body updated through REST. Push35327878801/job105545107463 and PR35327883884/job105545122680 are building. Existing Dependabot startup failure remains separate. APKs refreshed from clean0e7455a; signatures/hashes in artifacts/build-info.json.
- Stage6g while CI builds: added a real Android widget-host integration test for all3 providers, stopped/fresh timestamps, absent readings, Celsius/Fahrenheit and explicit ETA. It binds only the disposable development package using test shell permission, then removes its own host. Scripted values are identified in screenshot names. Compilation/test APK assembly PASSED1m13s (27 methods; /tmp/batstats-stage6g-compile.log); no runtime pass claimed. No local compiler remains active.
- Next: hold the stage6g push until current hosted results are inspected. Download ordinary/Shizuku reports and PNG/XML captures, visually review them, fix actual failures without weakening assertions, then publish the widget regression with verified corrections. Keep PR draft.
- PR updates use REST body from docs/PR_DESCRIPTION.md (`gh pr edit` uses a deprecated GraphQL field). Finish current APK provenance/offline guides and delivery after available device validation. After compaction perform recovery before implementation.
