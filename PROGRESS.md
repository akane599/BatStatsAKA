...
Next: finish the distribution-directory fix, verify its configured paths, and push it with the committed CI memory fix; inspect the resulting device reports/screenshots. Keep PR draft. API job-log download works when gh run log returns empty; use the jobs/<id>/logs endpoint. After any compaction perform recovery checks before implementation.
