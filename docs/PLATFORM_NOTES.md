...
The SDK catalog offers `system-images;android-36;google_apis_ps16k;x86_64` (revision7 at this checkpoint). [Android's16KB guidance](https://developer.android.com/guide/practices/page-sizes) includes an x86_64 emulator target; this simulates16KiB constraints on x86 hardware. A separate CI pass is prepared to verify process page size, native linking and the ordinary/Shizuku suites. It remains unexecuted, and cannot validate ARM64 or physical Samsung hardware.

## Screen event ordering
Android16 [Notifier](https://github.com/aosp-mirror/platform_frameworks_base/blob/android16-release/services/core/java/com/android/server/power/Notifier.java) updates interactive state separately from its asynchronous message/broadcast delivery (`updatePendingBroadcastLocked`, `sendWakeUpBroadcast`). A read can therefore see the new state before BatStats receives its broadcast. The two-second confirmation allowance is an app policy, not an Android delivery guarantee. It retains the first actual capture; late, missing or conflicting events leave the earlier interval unobserved. Contradictory queued screen events never override the current PowerManager reading. No extra wake lock, alarm or timer is introduced.
