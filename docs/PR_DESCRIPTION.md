...
- Lifecycle/screen-state sequencing, notification tap/stop/restart, all5 storage recovery cases, widgets, landscape checks and native loading passed on both4KiB and16KiB images. The16KiB phase initially reported an opaque `Process crashed.`; the captured log identified Android's low-memory killer terminating the foreground app on a2048M emulator, and the step now requests4096M.
