...
The host has no KVM and3.8GiB RAM plus4GiB swap. Builds and emulators must run sequentially. The initial ATD booted once with watchdog multiplier100; installation was interrupted. ATD omits SystemUI, so it cannot validate the notification shade. Its disposable AVD was removed to reclaim disk, keeping logs. Full API36 google_apis reached PackageManager initialization on5580 at384×800/dpi160 with enforced2560MiB RAM, sparse6GiB userdata and software rendering. Watchdog100 and shell UID2000 verified; Package Manager registration/completed boot not yet available. Manual snapshot saved after removing the unused task ATD image; emulator stopped before stage5c full Gradle checks. Snapshot restore is still unverified.
