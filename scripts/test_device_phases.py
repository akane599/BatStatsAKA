...
class DevicePhasesTest(unittest.TestCase):
    def run_phases(self, ordinary=0, shizuku=0, setup=0, boot="1"):
...
""")
    ...
    def test_both_phases_must_pass_and_keep_distinct_reports(self):
        ...
        reports = root / "app/build/reports/device-validation"
    ...
    def test_setup_failure_has_no_fictional_shizuku_results(self):
        ...
        self.assertFalse((root / "app/build/reports/device-validation/shizuku-results").exists())
        self.assertFalse((root / "app/build/reports/device-validation/shizuku-screenshots").exists())
